...
import { GetRemainByRouteTimeDTO, GetRemainNumberDTO, ShiftingRemainDto } from "../../cmd/dto";
...
export class TicketRemainService {
  ...
  private async getOrcreateTicket(ticketRemainId:string,shift:ShiftingRemainDto){
    let ticketRemain = await this.ticketRemainRepository.getById(ticketRemainId);
    if (ticketRemain) {
      return ticketRemain
    }
  ...
  async increaseTicketRemain(increase:ShiftingRemainDto): Promise<ticket_remain> {
    ...
    if (ticketRemain.ticket_remain_number+increase.amount > increase.maxTicket) {
      throw AppError.BadRequest("Cannot increasing Remain because it has reached its maximum value.");
    }
    
    // Increase the ticket remain number
    return this.ticketRemainRepository.increaseRemainNumber(ticketRemainId, increase.amount);
  }

  async decreaseTicketRemain(decrease:ShiftingRemainDto): Promise<ticket_remain> {
    ...
    return this.ticketRemainRepository.decreaseRemainNumber(ticketRemainId,decrease.amount);
  }
