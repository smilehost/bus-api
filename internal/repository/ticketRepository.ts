  ...
  async create(data: RouteTicketWithPrices) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        const pricesToCreate = data.list_route_ticket_price.map((priceItem) => ({
          route_ticket_price_ticket_id: routeTicket.route_ticket_id,
          route_ticket_price_type_id: priceItem.route_ticket_price_type_id,
          route_ticket_location_start: String(priceItem.route_ticket_location_start),
          route_ticket_location_stop: String(priceItem.route_ticket_location_stop),
          price: String(priceItem.price),
          route_ticket_price_route_id: parseInt(priceItem.route_ticket_price_route_id),
        }));
  ...
  async update(id: number, data: RouteTicketWithPrices) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        // 2. ลบราคาทั้งหมดก่อนแล้วค่อย insert ใหม่
        ...
        const newPrices = data.list_route_ticket_price.map((priceItem) => ({
          route_ticket_price_ticket_id: id,
          route_ticket_price_type_id: priceItem.route_ticket_price_type_id,
          route_ticket_location_start: String(priceItem.route_ticket_location_start),
          route_ticket_location_stop: String(priceItem.route_ticket_location_stop),
        ...
        await tx.route_ticket_price.createMany({
          data: newPrices,
        });
